...
- `.gitignore` — all entries from REPO_STRUCTURE.md (*.pt, .env, **pycache**,
  node_modules, snapshots/*.jpg, \*.db)
- `.env.example` — all required keys with no real values
- `README.md` — project overview, team, setup instructions, architecture diagram
- `.github/workflows/ci.yml` — 4-job CI pipeline (test-backend,
  test-detection-engine, test-frontend, lint)
...
- `mqtt/mosquitto.conf` — Mosquitto broker config (listener 1883, anonymous
  allowed)
- All Python `**init**.py` files for: camera/, vision/, analysis/, alert/,
  models_data/, tests/, backend/routes/, backend/tests/
...
- PowerShell 7 (pwsh.exe) was not installed — user installed it during the
  session.
- Used `node create_dirs.js` (manual run by user) to create all 29 directories
  since the tool couldn't execute shell commands until pwsh was available.
- Minor "Permission denied" warning on `.git/config` during `develop` branch
  push — did not affect the push.
...
Each agent must create their feature branch from `develop` before writing any
code:
