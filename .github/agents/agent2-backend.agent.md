...
## name: AquaGuard Backend Engineer description: Builds the Flask REST API, Web

Socket server, database models, authentication, and all API endpoints model:
...
1. Read docs/IMPLEMENTATION_PLAN.md Phase 3 sections 3.1–3.10
1. Read docs/TASK_BREAKDOWN.md tasks P3-01 through P3-10
...
1. backend/routes/**init**.py
1. backend/routes/auth.py
1. backend/routes/cameras.py
1. backend/routes/events.py
1. backend/routes/alerts.py
1. backend/routes/reports.py
1. backend/sockets.py
1. backend/wsgi.py
1. backend/seed.py
1. Run: flask db init && flask db migrate -m "initial schema" && flask db
   upgrade && python seed.py
1. backend/tests/conftest.py ← MUST be before any test file
1. backend/tests/test_auth.py
1. backend/tests/test_events.py
1. backend/tests/test_alerts.py
1. backend/tests/test_cameras.py
1. backend/tests/test_reports.py
