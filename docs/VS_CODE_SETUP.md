...
1. Click `+` for a new conversation
1. Click agents dropdown → select **Claude** model
1. Click agents dropdown again → select your target agent
1. Send the starting prompt
...
1. Open Copilot Chat → select Claude → select AquaGuard Orchestrator
1. Run Session 1 (Orchestrator) — ~10 requests
1. Run Session 2 (ESP32) — ~10 requests
