---
name: AquaGuard Documentation Engineer
description: Writes README.md, API reference, Swagger/OpenAPI spec, and developer setup guide
...
Never touch: `backend/` source code, `frontend/`, `detection_engine/` source code,
`esp32/`, `config/`, `.github/`
...
4. Read docs/IMPLEMENTATION_PLAN.md — understand what was built
5. Read docs/REPO_STRUCTURE.md — understand file layout
6. Read agents/status/agent1_done.md — CV engine deliverables
7. Read agents/status/agent2_done.md — API endpoints and WebSocket events
8. Read agents/status/agent5_done.md — real test results and latency numbers
...
2. **Repository setup** — clone, venv creation
3. **Python environment** — install all packages, verify CUDA
4. **MQTT broker** — install Mosquitto, run it, verify connection
5. **Environment variables** — copy `.env.example`, fill required values
6. **Database initialization** — flask db commands, seed script
7. **Frontend setup** — npm install, .env config
8. **ESP32 firmware** — open Arduino IDE, update config.h, flash steps
9. **Start all services** — use `scripts/start_dev.ps1` or manual steps
10. **Run detection engine** — python detection_engine/main.py
11. **Verify everything works** — run `scripts/verify_cuda.py`, open dashboard
...
2. **Component descriptions** — what each module does and why
3. **Data flow** — frame → detection → alert end-to-end
4. **Detection pipeline** — 5 stages with confidence scoring formula
5. **Alert dispatch** — 3 parallel threads (MQTT, API, logger)
6. **Database schema** — 5 tables with key fields
7. **MQTT topics** — aquaguard/alert, aquaguard/alert/reset, aquaguard/device/status
8. **WebSocket events** — alert_event, camera_status, system_status
9. **Key design decisions** — why YOLOv11, why MediaPipe, why Flask-SocketIO threading
