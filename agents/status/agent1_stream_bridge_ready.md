...
- No code changes were required in `detection_engine/` or `config/` for this
  re-run.
...
- `python -c "import detection_engine.main, detection_engine.alert.api_client,
  detection_engine.camera.registry, config.settings; print('import checks
  passed')"` → **1 passed, 0 failed**

## Blockers

- No functional blockers for CV-side stream bridge.
- Environment note: `.\aquaguard_env\Scripts\Activate.ps1` was used as the
  venv activation method in PowerShell context.
